...
export class ItemService {
  ...
  async createItem(createItemDto: CreateItemMasterDto) {
    try {
      ...
      if (error.code === 'P2002') {
        ...
        const target = error.meta?.target;
        console.error('P2002 error:', error, 'target:', target);

        // Only check for itemCode unique constraint
        if (
          (typeof target === 'string' && target.includes('itemCode')) ||
          (Array.isArray(target) && target.includes('itemCode'))
        ) {
          throw new ConflictException(
            `Item with code "${createItemDto.itemCode}" already exists. Please use a different item code.`,
          );
        ...
        throw new ConflictException(
          'Duplicate entry for itemCode. The item already exists. Please choose a different item code.',
        );
      } else if (error.code === 'P2003') {
        // ...existing foreign key error handling...
        const fieldName = error.meta?.field_name || 'unknown field';
        let friendlyMessage = 'Reference error: ';
        if (
          fieldName.includes('createdBy') ||
          fieldName.includes('updatedBy') ||
          fieldName.includes('deletedBy')
        ) {
          friendlyMessage = 'The specified user does not exist.';
        } else if (fieldName.includes('concernId')) {
          friendlyMessage = 'The specified concern/company does not exist.';
  ...
  private async findDuplicateFields(
    createItemDto: CreateItemMasterDto,
    targetFields: string[],
  ): Promise<string[]> {
  ...
  async update(id: number, updateItemDto: UpdateItemDto) {
    try {
      ...
      // Just use updateItemDto directly if PurchaseDetail is not part of the DTO
      const updatedItem = await this.prisma.itemMaster.update({
        where: { id },
        data: {
          ...updateItemDto,
      ...
      if (error.code === 'P2002') {
        ...
        if (
          ...
          throw new ConflictException(
            `Item with code "${updateItemDto.itemCode}" already exists. Please use a different item code.`,
          );
        }
